...
describe('parseRegExp', () => {
  ...
  const testCases = [
    ...
    [/^a|^b/, AST.union(AST.startMarker(undefined, str('b')), AST.startMarker(undefined, char('b')))],
    ...
    [/(?=a)b/, AST.concat(AST.positiveLookahead(char('a')), char('b'))], 
    [/(?=a)(?:b)/, AST.concat(AST.positiveLookahead(char('a')), char('b'))], 
    [/(?=a)(?=b)c/, AST.concat(AST.positiveLookahead(char('a')), AST.concat(AST.positiveLookahead(char('b')), char('c')))], 
    [/a(?=b)c/, AST.concat(char('a'), AST.concat(AST.positiveLookahead(char('b')), char('c')))], 
    [/a(?=b)/, AST.concat(char('a'), AST.positiveLookahead(char('b')))], 
    [/a(?=b)c(?=d)e/, AST.concat(char('a'), AST.concat(AST.positiveLookahead(char('b')), AST.concat(char('c'), AST.concat(AST.positiveLookahead(char('d')), char('e')))))], 
    // negative lookahead
    [/(?!a)b/, AST.concat(AST.negativeLookahead(char('a')), char('b'))], 
    [/(?!a)b|c/, AST.union(AST.concat(AST.negativeLookahead(char('a')), char('b')), char('c'))],
    // positive lookbehind
